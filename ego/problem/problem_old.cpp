...
namespace NEgo {
    ...
    {
        Model = Factory.CreateModel(ModelType);
        Model->InitWithConfig(spec.ModelConfig, Config.DimSize);

        Strategy.InitWithConfig(spec.StrategyConfig, Model);
    }
    ...
    void TProblem::SerialProcess(TSerializer& serial) {
        ...
        serial(ModelType, NEgoProto::TProblemState::kModelTypeFieldNumber);
        if (serial.IsInput()) {
            Model = Factory.CreateModel(ModelType);
